...
const command = {
    ...
    async execute(interaction: CommandInteraction) {
        try {
            const s = Object.entries(db.users).sort(([, a], [, b]) => (b as { points: number }).points - (a as { points: number }).points).slice(0, 10);
            await interaction.reply({ content: s.map((u, k) => `${k + 1}. <@${u[0]}>: ${(u[1] as { points: number }).points}`).join('\n') || 'Vide' });
        } catch (e) {
